...
AbvUdpInputFetcher::AbvUdpInputFetcher() : 
...
bool AbvUdpInputFetcher::startListening()
{
    mUdpServer->start();
    return true;  
}

void AbvUdpInputFetcher::onReceived(const std::string& message)
{
    convertThrusterCommandToForce(message); 
    Eigen::VectorXd latestInput(mThrustCmd.size());
    latestInput[0] = mThrustCmd[0]; 
    latestInput[1] = mThrustCmd[1]; 
    latestInput[2] = mThrustCmd[2]; 
    setLatestInput(latestInput);  
}

void AbvUdpInputFetcher::convertThrusterCommandToForce(const std::string& thrusterCommand)
{   
